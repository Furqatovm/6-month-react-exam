  ...
  useEffect(()=>{
      const getData = async () => {
          ...
          const result = await apiRequest({
            url: `https://${import.meta.env.VITE_BASE_URL}/api/student/get-all-students`,
            params:{
    ...
    <table className="w-full divide-y divide-gray-300">
      <thead className="">
        <tr>
          ...
          <th className="px-6 py-3 text-left text-sm font-medium text-gray-800">Telefon raqam</th>
          <th className="px-6 py-3 text-left text-sm font-medium text-gray-800">Guruhlar Soni</th>
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {
            ...
            <tr  className='flex items-center justify-center w-full h-full'>
             ...
             filteredData?.length > 0? filteredData.map((val) =>{
                ...
                <td className="px-6 py-4 whitespace-nowrap">{val.phone}</td>
                <td className="px-6 py-4 whitespace-nowrap">{val.groups.length.toLocaleString()}</td>
